...
def verify_imports() -> int:
    ...
    if blocked:
        ...
        say()
        say("  可以嘗試的方向（由快到慢）：")
        say("    1. 先看畫面上有沒有一個「Python 安全性許可」的通知視窗在等你回應。")
        say("       有的話按「同意」／「允許」，然後重新執行本安裝程式。")
        say("       （之前按過拒絕或關掉視窗的話，重跑就會再問一次。）")
        say("    2. 檢查 Smart App Control：設定 → 隱私權與安全性 →")
        say("       Windows 安全性 → 應用程式與瀏覽器控制 → 智慧型應用程式控制")
        say("    3. 把 Python 裝到 C:\\Program Files 之下（需要系統管理員），")
        say("       有些原則只允許非使用者可寫入的路徑")
        say("    4. 請 IT 將本專案資料夾與 Python 安裝路徑加入允許清單")
        say()
        say("  原始訊息：")
