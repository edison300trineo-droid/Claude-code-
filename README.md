...
| 刪除 | 該列右側「刪除」（會再確認一次） |
| 批次刪除 | 勾選最左欄的核取方塊 →「刪除勾選的案件」。點標題列的核取方塊可全選目前顯示的案件；先勾一列再按住 `Shift` 勾另一列可整段選取 |
| 搜尋 | 上方搜尋框，比對**案件編號／合約編號／研究編號／案件名稱／客戶名稱** |
...
| 案件名稱 | 選填，最長 200 字。一句話說明這件案子做什麼，例：`SD 大鼠 28 天重複劑量毒性試驗`。搜尋會比對這一欄 |
...
| POST | `/api/cases/bulk-delete` | 批次刪除，body 為 `{"ids": [1, 2, 3]}`，單次上限 500 筆，同一交易內完成 |
