...
DEMO_CASES = [
    ("TN-VI1141101-R01", "宏碩生技", "GLP 研究", "試驗執行中", "第 28 天中期報告", 5, "陳彥廷", "進行中",
     "SD 大鼠 28 天重複劑量毒性；動物房週報每週五更新。"),
    ("TN-BA1140902-M03", "光宇製藥", "方法確效", "QA 審查", "確效報告送 QA", -3, "林郁涵", "已延遲",
     "LC-MS/MS 血漿定量，選擇性再測一批。"),
    ("TN-BA1141015-S02", "英傑生醫", "生物分析", "檢體分析中", "第二批檢體完成分析", 12, "林郁涵", "進行中",
     ""),
    ("TN-QA1141020-A01", "誠泰藥業", "稽核", "報告草稿", "稽核報告初稿內部審閱", 2, "王孟儒", "需留意",
     "客戶要求同步提供 CAPA 建議。"),
    ("TN-OS1140815-C04", "北辰醫材", "外包試驗支援", "客戶審閱", "客戶回覆意見", 20, "張佩妤", "進行中",
     "外包至合作機構執行生物相容性試驗。"),
    ("TN-BD1141105-L01", "Meridian Bio", "BD 授權評估", "計畫書撰寫", "技術盡職調查會議", 9, "陳彥廷", "需留意",
     "NDA 已簽署，資料室 10/28 開放。"),
    ("TN-VI1140401-R09", "宏碩生技", "GLP 研究", "結案", "—", -40, "王孟儒", "已結案",
...
def cmd_seed_demo(args):
    ...
    if existing and not args.force:
        ...
        return 0
    today = date.today()
    for case_no, client, ctype, stage, milestone, offset, owner, status, notes in DEMO_CASES:
